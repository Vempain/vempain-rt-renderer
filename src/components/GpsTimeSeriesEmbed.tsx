...
import type {GpsBounds, GpsClusterItem, GpsPoint} from '../types';
...
function ClusterMarker({cluster, onExpand}: {
    cluster: GpsClusterItem;
    onExpand: (cluster: GpsClusterItem) => Promise<void>
}) {
...
export default function GpsTimeSeriesEmbed({identifier}: GpsTimeSeriesEmbedProps) {
    const {pageAPI} = useRendererRuntime();
    ...
    if (!overviewBounds) {
        return <Empty description="Kuvaston reittia ei loydy" image={Empty.PRESENTED_IMAGE_SIMPLE}/>;
    }
